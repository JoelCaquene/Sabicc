...
class DepositAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Informações do Usuário', {'fields': ('user', 'amount', 'payment_method', 'payer_name')}),
...
# --- CONFIGURAÇÕES DE SAQUE (BLOCO ATUALIZADO) ---
@admin.register(Withdrawal)
class WithdrawalAdmin(admin.ModelAdmin):
    # Alterado aqui para mostrar o valor convertido na lista principal
    list_display = ('user', 'amount', 'amount_converted', 'method', 'status', 'created_at')
    ...
    fieldsets = (
        ...
        ('Dados para Pagamento', {
            'fields': ('method', 'amount_converted', 'withdrawal_details'),
            'description': 'Estes são os dados para você realizar o envio na moeda correta.'
        }),
        ('Datas', {
            'fields': ('created_at',),
        }),
    )
    readonly_fields = ('created_at', 'amount_converted') # Deixamos como leitura apenas
